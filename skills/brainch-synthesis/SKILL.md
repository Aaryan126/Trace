...
You manage Brainch's synthesis agent. When a decision thread has been quiet (no new items) for a configurable window (default: 24 hours), this skill synthesizes a commit — a clear verdict and reasoning from the research session.
...
openclaw cron create --every 6h --name "brainch-synthesis" --session isolated \
  --system-event "Run Brainch synthesis: close quiet threads into commits"
```
