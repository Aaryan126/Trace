---
name: brainch-ingestion
description: Watches screenshots and browser history to capture decision-research activity for Brainch.
metadata:
  openclaw:
    requires:
      env: ["OPENAI_API_KEY"]
...
You manage the Brainch ingestion pipeline. This skill monitors the user's research activity by:
...
- On startup: ensure the Brainch service is running (`node packages/service/src/index.ts` from the Brainch project root)
- On user request: "start tracking", "watch my research", "begin ingestion"

## How to run

Use the `exec` tool to start the Brainch service process:
...
The service reads configuration from `~/.brainch/config.json` or environment variables:
- `BRAINCH_SCREENSHOT_DIR` — directory to watch (default: ~/Desktop)
- `BRAINCH_HISTORY_POLL_MS` — browser history poll interval in ms (default: 300000)
...
- No file modifications outside ~/.brainch/
